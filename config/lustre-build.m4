...
#
# Check whether we should build custom compression modules in utils/compression
#
AC_DEFUN([LB_CONFIG_COMPRESSION], [
AS_IF([test "x$enable_compression" = xyes],
	[AC_DEFINE(COMPRESSION_ENABLED, 1, [compression is enabled])])

AM_CONDITIONAL(COMPRESSION, [test x$enable_compression = xyes])

#
# Check whether or not we should build the LZ4 module in utils/compression
#
...
build_lz4_module="no"
AS_IF([test x$enable_compression = xyes -a x$have_kernel_lz4_compress_fast = xno -a x$have_kernel_lz4_compress = xno], [
	# compression is enabled but we don't have kernel LZ4, so we want to build our own LZ4
	build_lz4_module="yes"
])
AC_MSG_RESULT([$build_lz4_module])
AM_CONDITIONAL(LZ4, [test x$build_lz4_module = xyes])

]) # LB_CONFIG_COMPRESSION
