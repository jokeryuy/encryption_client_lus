...
/* TODO: adapt when/if chunk_desc shrinks */
void lustre_swab_chunk_desc(struct chunk_desc *cd)
{
    __swab64s(&cd->poffset);
    __swab64s(&cd->loffset);
    __swab32s(&cd->ppages);
    __swab32s(&cd->psize);
    __swab32s(&cd->lpages);
    __swab32s(&cd->lsize);
...
/* COMP */
void dump_cdesc(struct chunk_desc *cd)
{
    CDEBUG(D_RPCTRACE, "chunk_desc: lsize=%d, psize=%d, algo=%d, checksum=%x\n",
           cd->lsize, cd->psize, cd->algo, cd->cksum);
}
