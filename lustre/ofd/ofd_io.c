...
static int ofd_preprw_read(const struct lu_env *env, struct obd_export *exp,
			   ...
			   struct chunk_desc *cdesc, char *jobid)
{
	struct ofd_object *fo;
...
/* Twin-function for better debugging. Modified only last for-loop. TODO: To be merged with original */
static int ofd_preprw_write_compressed(const struct lu_env *env, struct obd_export *exp,
			    struct ofd_device *ofd, const struct lu_fid *fid,
			    ...
			    struct chunk_desc *cdesc, char *jobid)
{
	...
	rnb_offset = rnb->rnb_offset;
	for (*nr_local = i = 0, j = 0; i < obj->ioo_bufcnt; i++) {

		/* rc returns lpages in compressed case */
		rc = dt_bufs_get_compressed(env, ofd_object_child(fo),
				 rnb_offset + (loff_t)cdesc[i].loffset, rnb->rnb_len, lnb + j, &cdesc[i], dbt);
		...
		for (k = 0; k < cdesc[i].ppages; k++) {
			lnb[j+k].lnb_flags = rnb[i].rnb_flags;
			lnb[j+k].lnb_flags &= ~OBD_BRW_LOCALS;
			/* TODO: only first #ppages of lnb-pages get OBD_BRW_GRANTED flag, the remaining
				ones do not. Have they also been granted? Fix */
			if (!(rnb[i].rnb_flags & OBD_BRW_GRANTED))
				lnb[j+k].lnb_rc = -ENOSPC;
		}
		j += rc;
		*nr_local += rc;
		c_nr_local += cdesc[i].psize / PAGE_SIZE;
		if(cdesc[i].psize % PAGE_SIZE > 0)
			c_nr_local = c_nr_local +1;

		LASSERT(j <= PTLRPC_MAX_BRW_PAGES);
		tot_bytes += rnb[i].rnb_len;
		c_tot_bytes += cdesc[i].psize;
	}
			    ...
			    struct chunk_desc *cdesc, char *jobid)
{
	...
	if(cdesc != NULL)
		return ofd_preprw_write_compressed(env, exp, ofd, fid, la, oa,
				      objcount, obj, rnb, nr_local, lnb, cdesc, jobid);
	       ...
	       struct niobuf_local *lnb, struct chunk_desc *cdesc)
{
	...
	if (cmd == OBD_BRW_WRITE) {
		...
		rc = ofd_preprw_read(env, exp, ofd, fid, &info->fti_attr, oa,
				     obj->ioo_bufcnt, rnb, nr_local, lnb, cdesc,
				     jobid);
