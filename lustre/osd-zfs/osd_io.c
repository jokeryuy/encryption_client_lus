...
/* Twin-function for better debugging. Only parameters has been modified. Calculate with
	with lsizes and from rnb[0].ofsset. TODO: To be merged with original */
static int osd_bufs_get_compressed_write(const struct lu_env *env, struct osd_object *obj,
				loff_t off, ssize_t len, struct niobuf_local *lnb,
...
static int osd_bufs_get_compressed(const struct lu_env *env, struct dt_object *dt,
			loff_t offset, ssize_t len, struct niobuf_local *lnb,
			struct chunk_desc *cdesc,
			int rw)
{
	...
	if (rw == 0)
		{
			/* TODO: add read path
			rc = osd_bufs_get_compressed_read(env, obj, offset, len, lnb, cdesc);*/
		}
	else
		rc = osd_bufs_get_compressed_write(env, obj, offset, len, lnb, cdesc);
...
struct dt_body_operations osd_body_ops = {
	...
	.dbo_bufs_get_compressed        = osd_bufs_get_compressed,
