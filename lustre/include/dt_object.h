...
struct dt_body_operations {
	...
	int (*dbo_bufs_get_compressed)(const struct lu_env *env,
			    struct dt_object *dt,
			    loff_t pos,
			    ssize_t len,
			    struct niobuf_local *lb,
...
/* Twin-function for better debugging. Provide here explicitely offsets and lens.
	The offset of first rnb is not reachable in original. TODO: To be merged with original? */
static inline int dt_bufs_get_compressed(const struct lu_env *env, struct dt_object *d,
			      __u64 rnb_offset, __u64 rnb_len,
			      struct niobuf_local *lnb, struct chunk_desc *cdesc, int rw)
{
	LASSERT(d);
	LASSERT(d->do_body_ops);
	LASSERT(d->do_body_ops->dbo_bufs_get);

	return d->do_body_ops->dbo_bufs_get_compressed(env, d, rnb_offset, rnb_len, lnb, cdesc, rw);
}
