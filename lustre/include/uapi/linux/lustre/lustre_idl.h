...
/* Chunk descriptor. TODO: alignment */
/* TODO: check types, 32 or 64 bytes? This one is sent over the network,
	should be smaller at the expense of comfort? */
struct chunk_desc {
	__u64 	poffset;  	/* chunk offset of compressed chunks within RPC  - for comfort */
	__u64 	loffset;  	/* chunk offset of uncompressed chunks within RPC - for comfort */
	__u32 	ppages; 	/* number of physical compressed pages - for comfort */
	__u32	psize;		/* number of compressed bytes per chunk */
	__u32 	lpages; 	/* number of logical uncompressed pages - for comfort */
	__u32	lsize; 		/* number of uncompressed bytes per chunk */
	__u32	cksum; 		/* maybe for future ? */
	__u32	algo; 		/* used algorithm */
	__u32 	header;  	/* size of header (bytes) - for comfort */
};
